...
#pragma warning restore 0414
    ...
    // Compatibility wrappers for projects that used the original sample API.

    [Obsolete("Use Open() instead. The port is always opened for both reading and writing.")]
    public void OpenSerialPortWriteOnly()
    {
        Open();
    }

    [Obsolete("Subscribe to LineReceived, then call Open().")]
    public void OpenSerialPortReadWrite(Action<string> callback)
    {
        if (callback != null)
        {
    ...
    [Obsolete("Use Send() for raw text or SendLine() for a newline-terminated command.")]
